...
func TestAuthRegister(t *testing.T){
    ...
    if(errx != sql.ErrNoRows || "testcase" == username){
		//common.AsyncPrintln("TestAuthRegister: Removing test user first");
		common.GetDB().Exec("DELETE FROM account WHERE username=?","testcase");
	}

    // create a new user through auth
	//common.AsyncPrintln("TestAuthRegister: Adding new user");
	msg,err := Auth.AuthRegister("testcase", "testpwd", "eternialogic@gmail.com");
	
	if(err){
		common.AsyncPrintln("TestAuthRegister: Failed to add a new user!  MSG: "+msg);
		t.Fail();
	}
...
func TestAuth(t *testing.T){
	...
	// test for errors
	if(err){
		common.AsyncPrintln("TestAuth: FAIL: "+msg);
		t.Fail();
	}else{
		//common.AsyncPrintln("TestAuth: idtoken = "+idtoken);
	}
	
	// test the outputs:
	if(idtoken == ""){
		common.AsyncPrintln("TestAuth: FAIL: idtoken is not defined");
		t.Fail();
	}
	
	if(id == "0"){
		common.AsyncPrintln("TestAuth: FAIL: id is 0, it should be higher!");
		t.Fail();
	}
    ...
    if(auth_token != strings.ToUpper(idtoken)){
		common.AsyncPrintln("TestAuth: FAIL: idtoken is not placed in the table!");
		common.AsyncPrintln(auth_token,"!=",strings.ToUpper(idtoken))
		t.Fail();
	}
}
...
func TestAuthGetColumn(t *testing.T){	
	...
	//common.AsyncPrintln("TestAuthGetColumn: ID="+id,idtoken,msg,err);
	
	// now pull the username with that ID
	username,erry := common.GetRowColumn("account",id,"`username`");
	
	if(erry){
		common.AsyncPrintln("TestAuthGetColumn: FAIL USER ID not found");
		t.Fail();
	}
	
	if(username != "testcase"){
		common.AsyncPrintln("TestAuthGetColumn: FAIL output != input");
		t.Fail();
	}
}
...
func TestVerifyAuthLevel(t *testing.T){
	...
	if(!err){
		common.AsyncPrintln("TestVerifyAuthLevel: User=Admin? FAIL",msg);
		t.Fail();
	}
	...
	if(err){
		common.AsyncPrintln("TestVerifyAuthLevel: User!=User? FAIL",msg);
		t.Fail();
	}
	...
	if(err){
		common.AsyncPrintln("TestVerifyAuthLevel: Admin!=Admin? FAIL",msg);
		t.Fail();
	}
}
